...
export async function PATCH(request: Request) {
  ...
  const { name, brand_color } = await request.json()
  if (!name) {
  ...
  if (brand_color) body.brand_color = brand_color
  ...
  if (!updateRes.ok) {
    ...
    return NextResponse.json({ error: err.message ?? 'Failed to update agency' }, { status: 500 })
  }
